...
<script>
  ...
  import {requestChangeSchool} from "../../../api/api";
    ...
    methods: {
      ...
      onSubmit() {
        this.$refs.sizeForm.validate((valid) => {
          if (valid) {
            let changeSchoolParams = { schoolId: this.sizeForm.schoolId, schoolName: this.sizeForm.schoolName, principal: this.sizeForm.principal,
              schoolText: this.sizeForm.schoolText, schoolIcon: this.param1.schoolIcon};
            requestChangeSchool(changeSchoolParams).then(data => {
              console.log(data);
              let code = data.code;
              let msg = data.msg;
              if (code !== 200) {

...
<script>
  ...
  import {requestCreateBook} from "../../../api/api";
  export default {
    data() {
      return {
        fileList3: [],
    ...
    methods: {
      ...
      onSubmit() {
        this.$refs.sizeForm.validate((valid) => {
          if (valid) {
            let createBookParams = { bookName: this.sizeForm.bookName, bookRank: this.sizeForm.bookRank,
            subjectId: this.sizeForm.subjectId, bookIcon: this.param1.bookIcon};
            requestCreateBook(createBookParams).then(data => {
              //console.log(data);
              let code = data.code;
              let msg = data.msg;
              if (code !== 200) {

<template>
  <div>
    ...
    <el-form ref="sizeForm" :model="sizeForm" label-width="180px" size="mini">
      <el-form-item label="请输入要删除的教材编号">
...
<script>
  ...
  import {requestDeleteBook} from "../../../api/api";
  export default {
    data() {
      return {
        tableData3: [],
    ...
    methods: {
      onSubmit() {
        if (confirm("确定要删除该教材吗？")) {
          this.$refs.sizeForm.validate((valid) => {
            if (valid) {
              let deleteBookParams = {bookId: this.sizeForm.bookId};
              requestDeleteBook(deleteBookParams).then(data => {
                console.log(data);
